...
def get_ip(ip_type:int):
    global logger,ipv4_reg,ipv6_reg

    match ip_type:
        case 4|6:
            logger.debug("获取ip")
            try:
                request_text=requests.get("https://myip{}.ipip.net/".format(ip_type),proxies={"http":None,"https":None}).text
            except requests.Timeout:
                logger.error("获取ipv%s时链接超时",ip_type)
                return None
